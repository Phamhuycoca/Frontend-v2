...
const StoriesCard: React.FC = () => {
  return (
    <Card title="Stories" size='small' style={{ width: 300 }} bodyStyle={{ paddingBottom: 0 }}>
      <Space direction="vertical" style={{ width: '100%' }}>
        {stories.map((story) => (
          ...
          >
            ...
            <div>
              <Text strong style={{ display: 'block' }}>{story.name}</Text>
              <Text type="secondary" style={{ fontSize: 12 }}>{story.time}</Text>
            </div>
          </div>
        ))}
      </Space>
